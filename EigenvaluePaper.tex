...
% ||C^T C||_F^2
% =
% \sqrt{(\sigma_1^4 + \sigma_2^4) x^2 + 2\sigma_1^2\sigma_2^2 x(1-x)}
% $$
...
% ||\Sigma||
% \le
% \sqrt{((\sigma_1^2 + ||A||)^2 + \sigma_2^4) x^2 + 2\sigma_1^2\sigma_2^2 x(1-x)}
% $$

% We think that there are variables such that $x$ is large (generally following an increasing trend as $||E||$ increases), but will probably not be optimal for the Sparse PCA problem following our assumptions. Therefore, factor out $\sqrt{x}$ from the expression, which will be our 
